...
將所有變數投入訓練後，我們得出以下的多元線性迴歸方程式。此方程式量化了各項生理指標與預測目標（$\hat{Y}$，即存活時間）之間的線性關係：

$$
\begin{aligned}
\hat{Y} &= -1113.146 - 23.546(AU2) + 214.182(AU3) \\
        &\quad + 55.622(BCS) + 8.152(PI) + 8.993(ET) \\
        &\quad + 69.966(LT) + 1.300(AGE) + 22.476(Gender1)
\end{aligned}
$$
為了更直觀地評估模型體質，以下整理了該模型的關鍵效能指標與特徵顯著性：
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 my-8">
  ...
  <div class="bg-white border border-gray-200 p-6 rounded-xl shadow-sm">
    <div class="flex items-center gap-2 mb-4 border-b pb-2">
      ...
      <h4 class="text-lg font-bold text-gray-800 m-0">模型整體配適度</h4>
    </div>
    <ul class="space-y-3 text-sm text-gray-600 list-none pl-0">
      <li class="flex justify-between">
        <span class="font-medium">判定係數 (Adj. R²)</span>
        <span class="font-bold text-blue-600">0.7612</span>
      </li>
      <li class="text-xs text-gray-400 mt-1 mb-2">模型能解釋約 76.12% 的變異，具備良好的初始預測力。</li>
      
      <li class="flex justify-between">
        <span class="font-medium">殘差標準誤 (RSE)</span>
        <span class="font-bold text-gray-800">179.9</span>
      </li>
      <li class="text-xs text-gray-400 mt-1 mb-2">預測值與真實存活天數的平均絕對誤差。</li>
      
      <li class="flex justify-between">
        <span class="font-medium">整體顯著性 (p-value)</span>
        <span class="font-bold text-green-600">&lt; 2.2e-16</span>
      </li>
    </ul>
  </div>
  ...
  <div class="bg-gray-50 border border-gray-200 p-6 rounded-xl shadow-sm">
    <div class="flex items-center gap-2 mb-4 border-b pb-2">
      <span class="text-xl"></span>
      <h4 class="text-lg font-bold text-gray-800 m-0">特徵變數顯著性</h4>
    </div>
    <div class="space-y-4">
      <div>
        <div class="text-xs text-gray-500 font-bold mb-1">高度顯著驅動因子 (p &lt; 0.001)</div>
        <div class="flex flex-wrap gap-2">
          <span class="px-2 py-1 bg-white border border-green-200 text-green-700 text-xs font-mono rounded shadow-sm">PI</span>
          <span class="px-2 py-1 bg-white border border-green-200 text-green-700 text-xs font-mono rounded shadow-sm">ET</span>
          <span class="px-2 py-1 bg-white border border-green-200 text-green-700 text-xs font-mono rounded shadow-sm">AU3</span>
        </div>
      </div>
      <div>
        <div class="text-xs text-gray-500 font-bold mb-1">顯著影響因子 (p &lt; 0.05)</div>
        <div class="flex flex-wrap gap-2">
          <span class="px-2 py-1 bg-white border border-blue-200 text-blue-700 text-xs font-mono rounded shadow-sm">BCS</span>
          <span class="px-2 py-1 bg-white border border-blue-200 text-blue-700 text-xs font-mono rounded shadow-sm">LT</span>
        </div>
      </div>
      <div>
        <div class="text-xs text-gray-500 font-bold mb-1">無顯著影響 (建議後續剔除)</div>
        <div class="flex flex-wrap gap-2">
          <span class="px-2 py-1 bg-gray-200 text-gray-500 text-xs font-mono rounded">AU2</span>
          <span class="px-2 py-1 bg-gray-200 text-gray-500 text-xs font-mono rounded">AGE</span>
          <span class="px-2 py-1 bg-gray-200 text-gray-500 text-xs font-mono rounded">Gender1</span>
        </div>
      </div>
    </div>
  </div>
